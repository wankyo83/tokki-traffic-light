...
- 저장소의 `site/domains.json`·`site/status.json`은 현재 공개 Pages 결과보다 오래된 스냅샷입니다. **새 NAS 게시 시스템이 공개 중인 주소를 먼저 동기화하기 전에는 `site/**`를 배포하지 마세요.** Pages 작업도 오래된 스냅샷의 배포를 차단합니다.
